...
const userSchema = new mongoose.Schema({
  name: String, email: { type: String, unique: true }, password: String, phone: String, aadhar: String, ward: String, isVerified: { type: Boolean, default: true }, createdAt: { type: Date, default: Date.now }
});
...
const complaintSchema = new mongoose.Schema({
  ...
  feedbackText: String, ward: String, createdAt: { type: Date, default: Date.now }
});
...
app.post("/demo/seed", async (req, res) => {
    ...
    const demoData = [
      {
        ...
        latitude: 26.8500, longitude: 80.9490, areaName: "Hazratganj Main Crossing", ward: "Zone 1 - Central District", damageType: "Pothole", severity: "High",
      ...
      },
      {
        ...
        latitude: 26.8550, longitude: 80.9550, areaName: "Gomti Nagar Extension", ward: "Zone 3 - East Colony", damageType: "Drainage Block", severity: "High",
      ...
      },
      {
        ...
        latitude: 26.8400, longitude: 80.9400, areaName: "Alambagh Sector B", ward: "Zone 4 - South Extension", damageType: "Broken Streetlight", severity: "Medium",
      ...
      },
      {
        ...
        latitude: 26.8600, longitude: 80.9300, areaName: "Riverfront Bridge", ward: "Zone 1 - Central District", damageType: "Bridge Crack", severity: "High",
  ...
  } catch (error) { res.status(500).json({ message: "Error seeding data" }); }
});

// --- SUBMIT COMPLAINT ---
app.post("/complaint", upload.single("image"), async (req, res) => {
  try {
    const { latitude, longitude, areaName, damageType, severity, complaintText, ward } = req.body;
    ...
    const newComplaint = new Complaint({
      ...
      contractorName: "ABC Infra Pvt Ltd", verificationState: "Approved", ward: ward || "Unassigned"
    });
...
app.get("/complaint", async (req, res) => { try { const complaints = await Complaint.find(); res.status(200).json(complaints); } catch (e) { res.status(500).json({ message: "Error" }); }});
app.patch("/complaint/:id/upvote", async (req, res) => { try { const c = await Complaint.findById(req.params.id); c.upvotes += 1; await c.save(); res.status(200).json(c); } catch (e) { res.status(500).json({ message: "Error" }); }});
app.patch("/complaint/:id/simulate-resolve", async (req, res) => { try { const c = await Complaint.findById(req.params.id); c.status = "Resolved"; c.latestUpdate = "Contractor has completed the repair."; c.progressPercentage = 100; c.resolutionImageUrl = "https://images.unsplash.com/photo-1585250495376-78b1d355dc5b?auto=format&fit=crop&w=400&q=80"; c.resolutionText = "Area secured and infrastructure repaired."; await c.save(); res.status(200).json(c); } catch (e) { res.status(500).json({ message: "Error" }); }});
app.patch("/complaint/:id/rate", async (req, res) => { try { const c = await Complaint.findByIdAndUpdate(req.params.id, { rating: req.body.rating, feedbackText: req.body.feedbackText }, { new: true }); res.status(200).json(c); } catch (e) { res.status(500).json({ message: "Error" }); }});
app.post("/auth/login", async (req, res) => { try { const user = await User.findOne({ email: req.body.email }); if (!user || user.password !== req.body.password) return res.status(401).json({ message: "Invalid credentials" }); res.status(200).json({ user: { id: user._id, name: user.name, ward: user.ward } }); } catch (e) { res.status(500).json({ message: "Error" }); }});
app.post("/auth/register", async (req, res) => { try { const newUser = new User(req.body); await newUser.save(); res.status(201).json({ message: "Registered" }); } catch (e) { res.status(500).json({ message: "Error" }); }});
